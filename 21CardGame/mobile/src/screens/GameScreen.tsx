...
export function GameScreen({ navigation, route }: GameScreenProps) {
  const { numPlayers, playerName, playerAvatar, gameMode, resumeState, aiDifficulty, timedMode, roomCode, playerId } = route.params;
  const isMultiplayer = gameMode === 'private' || gameMode === 'random';

  const localEngine = useGameEngine();
  const multiplayerEngine = useMultiplayerGame({
    roomCode,
    playerId,
    numPlayers,
    playerName: playerName || 'Player',
    playerAvatar: playerAvatar || '😎',
    gameMode: (gameMode as 'private' | 'random') || 'private',
    enabled: isMultiplayer,
  });

  const gameEngine = isMultiplayer ? multiplayerEngine : localEngine;
  ...
  // Initialize game for practice mode (local)
  useEffect(() => {
  ...
  useEffect(() => {
    if (gameEngine.isGameOver && gameEngine.winner && !hasNavigatedToScoreboard.current) {
      ...
      if (!hasRecordedResult.current) {
        ...
        const localIndex = multiplayerEngine.localPlayerIndex;
        const humanWon = isMultiplayer
  ...
  const handleRematch = () => {
    if (isMultiplayer) {
      // For multiplayer, go back to waiting room for a new match
      navigation.navigate('Home');
      return;
    }
  ...
  const isMyTurn = isMultiplayer ? multiplayerEngine.isMyTurn : true;
  ...
  return (
    <View style={styles.container}>
      ...
      <GameBoard
        ...
        isMyTurn={isMyTurn}
        localPlayerIndex={isMultiplayer ? multiplayerEngine.localPlayerIndex : undefined}
      />
    </View>
  );
}
