...
export function GameBoard({ gameEngine, onNewGame, onRematch, isMultiplayer, isMyTurn = true, localPlayerIndex }: GameBoardProps) {
  ...
  // In multiplayer, remap so the local player is always at the bottom (position 0).
  // For local/practice mode, no remapping needed.
  const mpLocal = isMultiplayer ? (localPlayerIndex ?? 0) : 0;
  const remapIndex = (i: number): number => {
    if (i >= players.length) return -1;
    if (!isMultiplayer) return i;
    return (i + mpLocal) % players.length;
  };

  const player1 = remapIndex(0) >= 0 ? players[remapIndex(0)] : null;
  const player2 = remapIndex(1) >= 0 ? players[remapIndex(1)] : null;
  const player3 = remapIndex(2) >= 0 ? players[remapIndex(2)] : null;
  const player4 = remapIndex(3) >= 0 ? players[remapIndex(3)] : null;

  // Remapped current player index (relative to display positions)
  const displayCurrentIndex = isMultiplayer
    ? (currentPlayerIndex - mpLocal + players.length) % players.length
    : currentPlayerIndex;
  ...
  const lastPlayWasDragRef = React.useRef(false);
  ...
  const wrappedPlaySelectedToCenter = useCallback((pileIndex: number) => {
    lastPlayWasDragRef.current = false;
    snapshotHandSize();
  ...
  const handleCardDragEnd = useCallback((card: Card, source: CardSource, sourceIndex: number, dx: number, dy: number, moveX: number, moveY: number) => {
    ...
    lastPlayWasDragRef.current = true;
  ...
  React.useEffect(() => {
    if (lastEvent?.type === 'CARD_PLAYED' && lastEvent.destination.startsWith('Center') && !lastPlayWasDragRef.current) {
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.gameLayout}>
          {/* TOP PLAYER (opponent in 2p, or player at display index 1) */}
          {player2 && (
            <Animated.View
              style={[
                styles.topSection,
                displayCurrentIndex === 1 && { transform: [{ scale: turnPulseAnim }] },
              ]}
              key={`player-1-${player2.name}-${player2.avatar}`}
            >
              <PlayerArea
                player={player2}
                isCurrentPlayer={displayCurrentIndex === 1}
                selectedCard={displayCurrentIndex === 1 && !isCurrentPlayerAI && !isMultiplayer ? selectedCard : null}
                onSelectCard={displayCurrentIndex === 1 && !isCurrentPlayerAI && !isMultiplayer ? handleSelectCard : () => {}}
                onPlayToStorage={displayCurrentIndex === 1 && !isCurrentPlayerAI && !isMultiplayer ? playSelectedToStorage : () => {}}
                showHandCards={isMultiplayer ? false : displayCurrentIndex === 1}
                showStorageCards={true}
                compact={isMultiplayer ? true : displayCurrentIndex !== 1}
                position="top"
                newlyDrawnCards={getNewlyDrawnCardsForPlayer(remapIndex(1))}
                onEndTurn={displayCurrentIndex === 1 && !isCurrentPlayerAI && !isMultiplayer ? endCurrentTurn : undefined}
                canEndTurn={displayCurrentIndex === 1 && canEndCurrentTurn}
                onCancelSelection={displayCurrentIndex === 1 && !isCurrentPlayerAI && !isMultiplayer ? clearSelection : undefined}
                hasSelection={displayCurrentIndex === 1 && !!selectedCard}
                onCardDragEnd={displayCurrentIndex === 1 && !isCurrentPlayerAI && !isMultiplayer ? handleCardDragEnd : undefined}
                onCardDragStart={displayCurrentIndex === 1 && !isCurrentPlayerAI && !isMultiplayer ? handleCardDragStart : undefined}
              />
            </Animated.View>
          )}
          ...
          <View style={styles.middleRow} ref={middleRowRef} onLayout={handleMiddleRowLayout}>
            ...
            {player3 && player4 && (
              <View style={styles.leftSection} key={`player-2-${player3.name}-${player3.avatar}`}>
                <PlayerArea
                  player={player3}
                  isCurrentPlayer={displayCurrentIndex === 2}
                  selectedCard={displayCurrentIndex === 2 && !isCurrentPlayerAI && !isMultiplayer ? selectedCard : null}
                  onSelectCard={displayCurrentIndex === 2 && !isCurrentPlayerAI && !isMultiplayer ? handleSelectCard : () => {}}
                  onPlayToStorage={displayCurrentIndex === 2 && !isCurrentPlayerAI && !isMultiplayer ? playSelectedToStorage : () => {}}
                  showHandCards={isMultiplayer ? false : displayCurrentIndex === 2}
                  showStorageCards={true}
                  compact={isMultiplayer ? true : displayCurrentIndex !== 2}
                  position="left"
                  newlyDrawnCards={getNewlyDrawnCardsForPlayer(remapIndex(2))}
                  containerWidth={responsive.sideWidth}
                  containerHeight={middleRowHeight}
                  onEndTurn={displayCurrentIndex === 2 && !isCurrentPlayerAI && !isMultiplayer ? endCurrentTurn : undefined}
                  canEndTurn={displayCurrentIndex === 2 && canEndCurrentTurn}
                  onCancelSelection={displayCurrentIndex === 2 && !isCurrentPlayerAI && !isMultiplayer ? clearSelection : undefined}
                  hasSelection={displayCurrentIndex === 2 && !!selectedCard}
                />
              </View>
            )}
            ...
            <View style={styles.centerSection}>
              ...
              {isCurrentPlayerAI && !isMultiplayer && (
              ...
              {isMultiplayer && !isMyTurn && (
                <View style={styles.aiIndicator}>
                  <Text style={styles.aiIndicatorText}>
                    {language === 'he'
                      ? `ממתין ל${currentPlayer?.name || 'שחקן'}...`
            ...
            {(player4 || (player3 && !player4)) && (
              <View style={styles.rightSection} key={player4 ? `player-3-${player4.name}-${player4.avatar}` : `player-2-${player3!.name}-${player3!.avatar}`}>
                <PlayerArea
                  ...
                  isCurrentPlayer={player4 ? displayCurrentIndex === 3 : displayCurrentIndex === 2}
                  selectedCard={isMultiplayer ? null : (player4
                    ? (displayCurrentIndex === 3 && !isCurrentPlayerAI ? selectedCard : null)
                    : (displayCurrentIndex === 2 && !isCurrentPlayerAI ? selectedCard : null))}
                  onSelectCard={isMultiplayer ? () => {} : (player4
                    ? (displayCurrentIndex === 3 && !isCurrentPlayerAI ? handleSelectCard : () => {})
                    : (displayCurrentIndex === 2 && !isCurrentPlayerAI ? handleSelectCard : () => {}))}
                  onPlayToStorage={isMultiplayer ? () => {} : (player4
                    ? (displayCurrentIndex === 3 && !isCurrentPlayerAI ? playSelectedToStorage : () => {})
                    : (displayCurrentIndex === 2 && !isCurrentPlayerAI ? playSelectedToStorage : () => {}))}
                  showHandCards={isMultiplayer ? false : (player4 ? displayCurrentIndex === 3 : displayCurrentIndex === 2)}
                  showStorageCards={true}
                  compact={isMultiplayer ? true : (player4 ? displayCurrentIndex !== 3 : displayCurrentIndex !== 2)}
                  position="right"
                  newlyDrawnCards={player4 ? getNewlyDrawnCardsForPlayer(remapIndex(3)) : getNewlyDrawnCardsForPlayer(remapIndex(2))}
                  containerWidth={responsive.sideWidth}
                  containerHeight={middleRowHeight}
                  onEndTurn={isMultiplayer ? undefined : (player4
                    ? (displayCurrentIndex === 3 && !isCurrentPlayerAI ? endCurrentTurn : undefined)
                    : (displayCurrentIndex === 2 && !isCurrentPlayerAI ? endCurrentTurn : undefined))}
                  canEndTurn={player4 ? displayCurrentIndex === 3 && canEndCurrentTurn : displayCurrentIndex === 2 && canEndCurrentTurn}
                  onCancelSelection={isMultiplayer ? undefined : (player4
                    ? (displayCurrentIndex === 3 && !isCurrentPlayerAI ? clearSelection : undefined)
                    : (displayCurrentIndex === 2 && !isCurrentPlayerAI ? clearSelection : undefined))}
                  hasSelection={player4 ? displayCurrentIndex === 3 && !!selectedCard : displayCurrentIndex === 2 && !!selectedCard}
                />
              </View>
            )}
          </View>

          {/* BOTTOM PLAYER (local player — always show own cards in multiplayer) */}
          {player1 && (
            <Animated.View
              style={[
                styles.bottomSection,
                displayCurrentIndex === 0 && { transform: [{ scale: turnPulseAnim }] },
              ]}
              key={`player-0-${player1.name}-${player1.avatar}`}
            >
              <PlayerArea
                player={player1}
                isCurrentPlayer={displayCurrentIndex === 0}
                selectedCard={displayCurrentIndex === 0 && !isCurrentPlayerAI ? selectedCard : null}
                onSelectCard={displayCurrentIndex === 0 && !isCurrentPlayerAI ? handleSelectCard : () => {}}
                onPlayToStorage={displayCurrentIndex === 0 && !isCurrentPlayerAI ? playSelectedToStorage : () => {}}
                showHandCards={isMultiplayer ? true : displayCurrentIndex === 0}
                showStorageCards={true}
                compact={isMultiplayer ? false : displayCurrentIndex !== 0}
                position="bottom"
                newlyDrawnCards={getNewlyDrawnCardsForPlayer(remapIndex(0))}
                onEndTurn={displayCurrentIndex === 0 && !isCurrentPlayerAI ? endCurrentTurn : undefined}
                canEndTurn={displayCurrentIndex === 0 && canEndCurrentTurn}
                onCancelSelection={displayCurrentIndex === 0 && !isCurrentPlayerAI ? clearSelection : undefined}
                hasSelection={displayCurrentIndex === 0 && !!selectedCard}
                onCardDragEnd={displayCurrentIndex === 0 && !isCurrentPlayerAI ? handleCardDragEnd : undefined}
                onCardDragStart={displayCurrentIndex === 0 && !isCurrentPlayerAI ? handleCardDragStart : undefined}
              />
            </Animated.View>
          )}
      ...
      {/* Interaction blocker for multiplayer when it's not your turn */}
      {isMultiplayer && !isMyTurn && !isGameOver && (
        <View
          style={{
            position: 'absolute',
      ...
      {/* Confetti only when the local player wins */}
      <ConfettiAnimation visible={isGameOver && !!winner && (isMultiplayer ? winner.playerNumber === (localPlayerIndex ?? 0) + 1 : !winner.isAI)} />
