...
import { playCardPlaySound, playInvalidMoveSound, playPileCompleteSound } from '../utils/sounds';
...
export interface UseGameEngineReturn {
  ...
  loadState: (state: GameState, options?: { skipPrevPlayerUpdate?: boolean }) => void;
  getHint: () => AIMove | null;
}

interface UseGameEngineOptions {
  skipDelayedDraw?: boolean;
}

/**
 * React hook for managing the game engine state
 */
export function useGameEngine(engineOptions?: UseGameEngineOptions): UseGameEngineReturn {
  ...
  // In multiplayer, only the host should draw cards (skipDelayedDraw for non-host).
  useEffect(() => {
    if (engineOptions?.skipDelayedDraw) {
      logger.debug('useGameEngine: Delayed draw SKIPPED (non-host)');
      return;
    }
    ...
    logger.debug(`useGameEngine: Delayed draw effect - prevIdx=${prevPlayerIndexRef.current}, curIdx=${currentPlayerIndex}, changed=${playerChanged}`);
    ...
    const timerId = setTimeout(() => {
      setGameState(prevState => {
        ...
        if (!player || player.hand.length >= 5) {
          logger.debug(`useGameEngine: Delayed draw - player ${player?.name} already has ${player?.hand.length} cards, skipping`);
          return prevState;
        }
        logger.debug(`useGameEngine: Delayed draw REFILLING ${player.name} (hand: ${player.hand.length} → 5)`);
        return drawCardFromStockDelayed(prevState, currentPlayerIndex);
      });
    }, 500);
    delayedDrawTimeoutRef.current = timerId;
  }, [gameState.currentPlayerIndex, gameState.gameStarted, gameState.gameOver, engineOptions?.skipDelayedDraw]);
  ...
  const loadState = useCallback((state: GameState, options?: { skipPrevPlayerUpdate?: boolean }) => {
    logger.debug(`useGameEngine: loadState - currentPlayerIndex=${state.currentPlayerIndex}, skipPrevPlayerUpdate=${!!options?.skipPrevPlayerUpdate}, prevRef=${prevPlayerIndexRef.current}`);
    setGameState(state);
    setSelectedCard(null);
    if (!options?.skipPrevPlayerUpdate) {
